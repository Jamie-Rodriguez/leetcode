...
const maxProfitTopDown = prices => {
    // Booleans will be cast to ints: false -> 0, true -> 1
    const hash = (a, b) => (a + b) * (a + b + 1) / 2 + b
    const cache = new Map()

    const getMaxProfit = (canBuy, day) => {
        if (day >= prices.length)
            return 0

        const stateHash = hash(canBuy, day)

        if (cache.has(stateHash))
            return cache.get(stateHash)

        // AKA "cooldown"
        const doNothing = getMaxProfit(canBuy, day + 1)

        if (canBuy) {
            const buy = getMaxProfit(false, day + 1) - prices[day]
            cache.set(stateHash, Math.max(buy, doNothing))
        } else {
            const sell = getMaxProfit(true, day + 2) + prices[day]
            cache.set(stateHash, Math.max(sell, doNothing))
        }
...
const maxProfit = prices => {
    ...
    const subproblems = Array(prices.length + 2).fill(0)

    for (let buyDay = prices.length - 1; 0 <= buyDay; buyDay--) {
        let maxProfit = 0

        // Get maximum profit from selling the currently bought stock in the future
        for (let sellDay = buyDay + 1; sellDay < prices.length; sellDay++)
            maxProfit = Math.max(maxProfit,
                                 prices[sellDay] - prices[buyDay] + subproblems[sellDay + 2])

        // Compare with not selling the current stock and going to the next day
        subproblems[buyDay] = Math.max(subproblems[buyDay + 1], maxProfit)
    }
