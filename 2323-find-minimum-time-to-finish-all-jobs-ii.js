...
const minimumTime = (jobs, workers) => {
    const sortAsc = (a, b) => a - b
    const sortedJobs = jobs.sort(sortAsc)
    const sortedWorkers = workers.sort(sortAsc)

    let time = 0

    for (let i = 0; i < sortedJobs.length; i++)
        time = Math.max(Math.ceil(sortedJobs[i] / sortedWorkers[i]), time)

    return time
}


console.assert(minimumTime([5, 2, 4], [1, 7, 5]) === 2)
console.assert(minimumTime([3, 18, 15, 9], [6, 5, 1, 3]) === 3)
